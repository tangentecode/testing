...
void set_wallpaper(const char *path) {
    char command[512];
    snprintf(command, sizeof(command), "gsettings set org.gnome.desktop.background picture-uri 'file://%s'", path);
    system(command);
}
#elif __APPLE__
void set_wallpaper(const char *path) {
    char command[512];
    snprintf(command, sizeof(command), "osascript -e 'tell application \"Finder\" to set desktop picture to POSIX file \"%s\"'", path);
    system(command);
}
#else
void set_wallpaper(const char *path) {
